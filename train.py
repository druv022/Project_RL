...
def compute_target(model, reward, next_state, done, discount_factor):
    ...
    if not non_terminal_states.nelement() == 0:
        next_state_values[right_index],_ = model(non_terminal_states).max(1)
    
    target = reward + discount_factor*next_state_values
    
    return target.unsqueeze(1)

def train(model, memory, optimizer, batch_size, discount_factor, beta=None):
...
def main():
    ...
    for i in tqdm(range(ARGS.num_episodes)):
        ...
        while not done:
            ...
            loss = train(model, replay, optimizer, ARGS.batch_size, ARGS.discount_factor, beta=beta)
